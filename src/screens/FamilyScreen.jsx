        ...
        <motion.div className="flex flex-col gap-7 px-5 pt-5 pb-8" variants={container} initial="hidden" animate="show">
          ...
          <motion.div variants={item} className="grid grid-cols-3 gap-2">
            {statCards.map((card, i) => (
              <div key={i} className="glass-card flex flex-col items-center text-center" style={{
                ...
                <span className="font-work-sans" style={{ fontSize: card.highlight ? '14px' : '18px', fontWeight: 600, color: card.highlight ? '#A07020' : '#1A1410', marginBottom: '4px' }}>
                  {card.value}
                </span>
                <span className="font-work-sans" style={{ fontSize: '9px', fontWeight: 400, color: 'rgba(26,20,16,0.54)', lineHeight: 1.4 }}>{card.label}</span>
              </div>
            ))}
          </motion.div>
          ...
          <motion.div variants={item} className="flex flex-col items-center gap-5 py-8">
            ...
            <div className="flex flex-col items-center gap-1.5">
              <p className="font-work-sans" style={{ fontSize: '18px', fontWeight: 400, color: '#1A1410', margin: 0 }}>{familyStats.currentCeremony} ceremony</p>
              <p className="font-work-sans" style={{ fontSize: '12px', fontWeight: 500, color: '#A07020', margin: 0, letterSpacing: '0.04em' }}>{familyStats.currentCeremonyStatus}</p>
            </div>
          </motion.div>
