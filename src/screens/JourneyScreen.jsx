      ...
      <div className="flex flex-col h-full overflow-y-auto no-scrollbar">
        ...
        <motion.div className="flex flex-col gap-5 px-5 pb-4" variants={container} initial="hidden" animate="show">
          <motion.div variants={item} className="mt-2 text-center">
            <h1 className="font-cormorant italic" style={{ fontSize: '32px', color: '#1A1410', fontWeight: 300, lineHeight: 1.1, margin: 0, letterSpacing: '-0.02em' }}>
              Your wedding journey
            </h1>
            <p className="font-outfit" style={{ fontSize: '13px', fontWeight: 300, color: 'rgba(26,20,16,0.42)', margin: '5px 0 0' }}>
                ...
                <div key={phase.id}>
                  <div className="glass-card" style={{
                    ...
                    <div className="flex items-center gap-3 mb-1">
                      ...
                      <span className="font-outfit flex-1" style={{ fontSize: '13px', fontWeight: isActive ? 500 : 300, color: isDone ? 'rgba(26,20,16,0.3)' : '#1A1410' }}>
                        {phase.name}
                      </span>
                      {isActive && <span className="font-outfit" style={{ fontSize: '9px', fontWeight: 600, color: '#A07020', background: 'rgba(200,151,58,0.1)', border: '1px solid rgba(200,151,58,0.25)', padding: '2px 9px', borderRadius: '99px', letterSpacing: '0.06em' }}>In progress</span>}
                      {isDone && <span className="font-outfit" style={{ fontSize: '10px', fontWeight: 300, color: 'rgba(45,96,37,0.65)' }}>{phase.completed}/{phase.tasks}</span>}
                      {isUpcoming && <span className="font-outfit" style={{ fontSize: '10px', fontWeight: 300, color: 'rgba(26,20,16,0.28)' }}>{phase.tasks} tasks</span>}
                    </div>

                    {isActive && (
                      <div className="mt-3">
                        <div className="flex justify-between mb-1.5">
                          <span className="font-outfit" style={{ fontSize: '10px', fontWeight: 300, color: 'rgba(26,20,16,0.38)' }}>{phase.completed} of {phase.tasks} complete</span>
                          <span className="font-outfit" style={{ fontSize: '10px', fontWeight: 500, color: '#A07020' }}>{Math.round(progress)}%</span>
                        </div>
                        <div style={{ height: '3px', background: 'rgba(0,0,0,0.07)', borderRadius: '99px', overflow: 'hidden' }}>
                  ...
                  {isActive && phase.aiNudge && (
                    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7, duration: 0.4 }}
                      ...
                      <div className="flex flex-col gap-2.5 flex-1">
                        <p className="font-outfit" style={{ fontSize: '12px', fontWeight: 400, color: 'rgba(26,20,16,0.72)', margin: 0 }}>{phase.aiNudge}</p>
                        <button className="inline-flex self-start font-outfit" style={{ background: 'linear-gradient(135deg, #7A0F46 0%, #5C0B35 100%)', color: '#FFFFFF', fontSize: '11px', fontWeight: 500, padding: '8px 15px', borderRadius: '10px', border: 'none', cursor: 'pointer', minHeight: '44px', boxShadow: '0 4px 12px rgba(122,15,70,0.25)' }}>
                          Review now →
                        </button>
                      </div>
                    </motion.div>
          ...
          <motion.div variants={item}>
            <button onClick={() => navigate('/website')} className="w-full glass-card flex items-center gap-4" style={{ padding: '16px 18px', border: '1px solid rgba(200,151,58,0.28)', cursor: 'pointer', textAlign: 'left' }}>
              ...
              <div className="flex flex-col flex-1">
                <span className="font-outfit" style={{ fontSize: '13px', fontWeight: 500, color: '#1A1410' }}>Wedding Website</span>
                <span className="font-outfit" style={{ fontSize: '11px', fontWeight: 300, color: 'rgba(26,20,16,0.42)', marginTop: '2px' }}>Build and share your wedding page with guests</span>
              </div>
              <span className="font-outfit" style={{ fontSize: '14px', color: 'rgba(26,20,16,0.3)' }}>→</span>
            </button>
          </motion.div>
